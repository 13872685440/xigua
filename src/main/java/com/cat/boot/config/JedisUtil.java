...
public class JedisUtil {
	...
	public String getMinioPath(String key) {
		return hget(RedisConst.minio_path_key, key, RedisConst.path_db);
	}
	
	public String getDict_id(String key) {
		return hget(RedisConst.dict_id, key, RedisConst.path_db);
	}
	
	public void addLog(HttpServletRequest request,String userName) {
		JSONObject j = new JSONObject();
		j.put("userName", userName);
		j.put("date", CalendarUtil.getYyyyMmDdHHmmss(Calendar.getInstance()));
		j.put("service", request.getRequestURI());
		j.put("remoteAddr", request.getRemoteAddr());
		j.put("remotePort", request.getRemotePort());
		lpush(RedisConst.log_db, "log_temp_" + CalendarUtil.getYyyyMmDd(Calendar.getInstance()), 
				j.toJSONString());
		lpush(RedisConst.log_db, userName+"_log_temp_" + CalendarUtil.getYyyyMmDd(Calendar.getInstance()), 
				j.toJSONString());
	}
	
	public void addLoginLog(HttpServletRequest request,String userName,String type) {
		JSONObject j = new JSONObject();
		j.put("userName", userName);
		j.put("date", CalendarUtil.getYyyyMmDdHHmmss(Calendar.getInstance()));
		j.put("service", request.getRequestURI() + suffix(type));
		j.put("remoteAddr", request.getRemoteAddr());
		j.put("remotePort", request.getRemotePort());
		lpush(RedisConst.log_db, "log_temp_" + CalendarUtil.getYyyyMmDd(Calendar.getInstance()), 
				j.toJSONString());
	...
	private String suffix(String type) {
		switch (type) {
		case "web":
			return "（web端）";
